...
export const getCommitsOverTime = async (username, repo) => {
  ...
  const dbRes = await retrieveFromDB(username, "commits");
  if (dbRes[0] !== -1) {
    return dbRes;
  } else {
    let resArr = [];
    ...
    await storeToDB(username, ret, "commits");
    console.log(ret);
    return ret;
  }
}
...
export const getAdditionsDeletionsRatios = async (username, repo) => {
  ...
  const dbRes = await retrieveFromDB(username, "changes");
  if (dbRes[0] !== -1) {
    return dbRes;
  } else {
    ...
    await storeToDB(username, ret, "changes");
    return ret;
  }
